...
list_responses  = colnames(df)[c(25)] # pco2_bacterivore
...
#  Setting
#-------------------------------------------
if(plot_to_pdf) pdf("output/modeling_rf/RF_bacterivore_pco2/partial_dependence_plots.pdf", paper="a4r", width=9.5, height=7, pointsize=10)  # start printing to pdf
...
write.csv(round(result_summary, digits = 3), "output/modeling_rf/RF_bacterivore_pco2/variable_importance.csv", row.names = TRUE)
write.csv(round(result_summary_pval, digits = 5), "output/modeling_rf/RF_bacterivore_pco2/p-value.csv", row.names = TRUE)
...
save.image(file="output/modeling_rf/RF_bacterivore_pco2/RF_output_RF_bacterivore.RData")
