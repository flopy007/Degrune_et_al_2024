...
list_responses  = colnames(df)[c(17)] # pco1_c (cercozoa)
...
#  Setting
#-------------------------------------------
if(plot_to_pdf) pdf("output/modeling_rf/RF_cercozoa/partial_dependence_plots.pdf", paper="a4r", width=9.5, height=7, pointsize=10)  # start printing to pdf
...
write.csv(round(result_summary, digits = 3), "output/modeling_rf/RF_cercozoa/variable_importance.csv", row.names = TRUE)
write.csv(round(result_summary_pval, digits = 5), "output/modeling_rf/RF_cercozoa/p-value.csv", row.names = TRUE)
...
save.image(file="output/modeling_rf/RF_cercozoa/RF_output_cercozoa.RData")
