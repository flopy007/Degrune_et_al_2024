...
# _____create the output directory_____
...
list_responses  = colnames(df)[c(20)] # pco1_bacterivore
list_predictors = colnames(df)[-c(1:5, 17, 19:20)] # abiotic factors + pco1_f + pco1_b
...
#  Setting
#-------------------------------------------
if(plot_to_pdf) pdf("output/modeling_rf/RF_bacterivore/partial_dependence_plots.pdf", paper="a4r", width=9.5, height=7, pointsize=10)  # start printing to pdf
...
write.csv(round(result_summary, digits = 3), "output/modeling_rf/RF_RF_bacterivore/variable_importance.csv", row.names = TRUE)
write.csv(round(result_summary_pval, digits = 5), "output/modeling_rf/RF_RF_bacterivore/p-value.csv", row.names = TRUE)
...
save.image(file="output/modeling_rf/RF_RF_bacterivore/RF_output_RF_bacterivore.RData")
